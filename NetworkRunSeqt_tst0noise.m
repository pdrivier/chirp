function [conn,vbar,veg,lfp,tspE,tspI,Isynbar,inp,seqs]= NetworkRunSeqt_tst0noise(p,input,NE,NI,T,opt)
...
mn = p.gmaxEE/NE; %PR: mean of gaussian dist of possible syn values
vr = p.gvarEE*mn;  
GEE = sqrt(vr)*W+mn; %var(aX) = a^2var(X)
GEE(GEE<0) = 0;
conn.EtoE = GEE; %PR: aggregate connectivity matrices
...
for secNum = 0:T % original secNum = 1:T (which made the simulation begin at 1sec)
    tic
    ...
    for idt = 1:length(t)
        if and(mod(idt,1000)==1, idt>1)
            ids = (idt-1)/1000+1+1000*(secNum); %PR: original: ids = (idt-1)/1000+1+1000*(secNum-1);
            vE(vE>0) = 50;
            vI(vI>0) = 50;
        ...
        if ~isempty(idX)
            ...
            for idn = 1:length(idX)
                n = idX(idn);
                tspE.times(end+1) =  t(idt)/1000+secNum; %PR: original: tspE.times(end+1) =  t(idt)/1000+secNum-1;
                tspE.celln(end+1) = n;
            end
        end
        % I cells
        ...
        if ~isempty(idX)
            ...
            for idn = 1:length(idX)
                n = idX(idn);
                tspI.times(end+1) =  t(idt)/1000+secNum; %PR: original: tspI.times(end+1) =  t(idt)/1000+secNum-1;
                tspI.celln(end+1) = n;
            end
        end
    end
