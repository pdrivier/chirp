function [p] = defaultparamsCA1_weakPING_gmaxEEIdc20pctincgmaxII()
%increase gmaxEE + increase direct current to inhibitory neurons + increase 
%gmaxII to synchronize your inhibitory population without having to totally 
%crank up the direct current + slightly increase the direct current to 
%pyramidal neurons to achieve 6 Hz oscillation in the firing rate figures

%here, Idc is 20 percent increase from original value
...
p.gmaxII = 5/(60/100); %MOD: increase from =3/(80/100)
p.gmaxEI = 2/(30/100);
p.gmaxIE = 5/(60/100);
p.gmaxEE = p.gmaxIE/2; %MOD: increase from =p.gmaxEI/10; 1on1 syn strength /percent synch efficacy
...
p.Edc = 40;
p.jmpE = 210;
p.Idc = 180+(180*0.20); %MOD: increase from =180
p.jmpI = 700;
